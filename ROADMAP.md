...
- [x] レイヤ骨格を作る（`presentation / application / domain / infrastructure / shared`、[architecture §3](./docs/design/architecture.md)）
...
- [ ] Editor（island）: プレーンテキスト、自動保存 / 手動保存 / 文字数 / Preview（PRD §11, [frontend §5](./docs/design/frontend.md)）
- [ ] 記法変換: ルビ `｜文章《ルビ》` / 傍点 `《《文章》》`（サーバ側変換 + XSS 対策、[frontend §4.2](./docs/design/frontend.md)）
...
- [ ] Docker: `Dockerfile.prod` + `docker-compose.prod.yml` + `cloudflared` 本番構成（[infrastructure §4, §7](./docs/design/infrastructure.md)）
...
- `redis` / `worker` / `analytics-worker` を Compose に追加（PRD §54、[infrastructure §3.2](./docs/design/infrastructure.md)）
- Analytics を独立サービスへ切り出し（[architecture §10](./docs/design/architecture.md)）
